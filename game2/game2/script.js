...
function makeGhostFromCord(cordEl) {
  ...
  if (cordId) ghost.setAttribute("data-cord-id", cordId);
...
function resetAll() {
  ...
  setStatus("Добро пожаловать! Перетащите нужный патч‑корд на стену справа.");
}
  ...
  document.querySelectorAll("[data-action]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (action === "check") {
        ...
        setStatus(
          hasCorrect
            ? "Выбран правильный патч корд"
            : "Выбран неправильный патч корд (сообщение об ошибке проработаем позже).",
        );
      }
    });
  });
  ...
  cords.forEach((cord) => {
    cord.addEventListener("pointerdown", (ev) => {
      if (isPlacementLocked) {
        setStatus("Нажмите \"сброс\", чтобы выбрать другой патч-корд");
        return;
      }
